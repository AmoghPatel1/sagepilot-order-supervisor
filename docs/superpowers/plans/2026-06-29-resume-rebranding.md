# Resume Rebranding Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rename project to "AI Order Supervisor" and strip all assessment/company-specific framing from docs and code.

**Architecture:** Pure text and string edits across six files. No code logic changes. No tests required — verification is grep-based to confirm zero remaining references.

**Tech Stack:** Markdown, Python (FastAPI), TypeScript (Next.js)
...
| File | Change Type |
|---|---|
| `backend/app/main.py` | String — FastAPI title |
| `frontend/app/page.tsx` | String — h1 text |
| `frontend/app/layout.tsx` | String — metadata title + description |
| `ARCHITECTURE.md` | String — first line rename |
| `README.md` | Edit — title, showcase intro, drop POC, folder path |
| `DESIGN_NOTE.md` | Full rewrite — strip assessment framing |

---

## Task 1: Code String Changes
...
- [ ] **Step 1: Update FastAPI title in `backend/app/main.py`**
...
- [ ] **Step 2: Update h1 in `frontend/app/page.tsx`**
...
- [ ] **Step 3: Update metadata in `frontend/app/layout.tsx`**
...
Run (PowerShell):
```powershell
Select-String -Path "backend\app\main.py","frontend\app\page.tsx","frontend\app\layout.tsx" -Pattern "Sagepilot|sagepilot|Create Next App|Generated by create"
```
Expected: no matches.
...
```bash
git add backend/app/main.py frontend/app/page.tsx frontend/app/layout.tsx
git commit -m "rebrand: rename to AI Order Supervisor in code files"
```

---

## Task 2: ARCHITECTURE.md First Line

**Files:**
- Modify: `ARCHITECTURE.md`

- [ ] **Step 1: Update first line of `ARCHITECTURE.md`**
...
Run (PowerShell):
```powershell
Select-String -Path "ARCHITECTURE.md" -Pattern "Sagepilot|sagepilot"
```
Expected: no matches.
...
git commit -m "rebrand: rename to AI Order Supervisor in ARCHITECTURE.md"
```
...
- [ ] **Step 1: Replace title and opening paragraph**
...
- [ ] **Step 2: Update project structure path**
...
Run (PowerShell):
```powershell
Select-String -Path "README.md" -Pattern "Sagepilot|sagepilot|POC"
```
Expected: no matches.
...
```bash
git add README.md
git commit -m "rebrand: rewrite README opening and rename to AI Order Supervisor"
```

---

## Task 4: DESIGN_NOTE.md Full Rewrite

**Files:**
- Modify: `DESIGN_NOTE.md`

- [ ] **Step 1: Replace entire contents of `DESIGN_NOTE.md`**
...
- [ ] **Step 2: Verify no assessment framing remains**

Run (PowerShell):
```powershell
Select-String -Path "DESIGN_NOTE.md" -Pattern "assignment|the spec|POC|Sagepilot|sagepilot|being evaluated|2-day|2-4 day"
```
Expected: no matches.
...
git commit -m "rebrand: rewrite DESIGN_NOTE.md to remove assessment framing"
```
...
- [ ] **Step 1: Confirm zero remaining brand/assessment references across all non-gitignored files**

Run (PowerShell):
```powershell
Get-ChildItem -Recurse -Include "*.md","*.py","*.ts","*.tsx","*.sql","*.json" |
  Where-Object { $_.FullName -notmatch "\\venv\\" -and $_.FullName -notmatch "\\node_modules\\" -and $_.FullName -notmatch "\\package-lock\\" -and $_.FullName -notmatch "\\specs\\" -and $_.FullName -notmatch "\\plans\\" } |
  Select-String -Pattern "Sagepilot|sagepilot|POC|the spec|the assignment|being evaluated|2-day POC|Create Next App"
```
Expected: no matches (the specs/plans docs are excluded since they document the process, not the product).

- [ ] **Step 2: Spot-check browser tab title**

Start frontend (`cd frontend && npm run dev`), open `http://localhost:3000` in browser.
Expected: browser tab reads "AI Order Supervisor".

- [ ] **Step 3: Spot-check API docs title**

Start backend (`cd backend && uvicorn app.main:app --reload --port 8000`), open `http://localhost:8000/docs`.
Expected: page heading reads "AI Order Supervisor".
